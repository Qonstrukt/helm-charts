...
A Helm Chart for generic PHP applications on Kubernetes.

## TL;DR

```bash
helm repo add qonstrukt https://qonstrukt.github.io/helm-charts/
helm install my-php-app qonstrukt/php
```

## Introduction

This Chart uses [these very complete PHP images](https://github.com/Qonstrukt/docker-images-php) to deploy a generic PHP application to Kubernetes.
...
To install the chart with the release name `my-php-app`:
...
To uninstall/delete the `my-php-app` deployment:

```bash
helm uninstall my-php-app
```
...
| image.repository | string | `"qonstrukt/php"` | Image name |
...
| containerPort | int | `80` | Internal http container port |
...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| service.port | int | `80` | Apache or PHP-FPM service port |
...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| ingress.enabled | bool | `false` | Enable ingress for web service |
